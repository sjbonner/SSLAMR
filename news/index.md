...
- Revised convergence metrics. Convergence is now assessed through two
  factors:

1.  the similarity of the probability of presence across the chains for
    each candidate and 2) the Brooks-Gelman-Rubin diagnostic for the
    non-zero beta values for each candidate.

## SSLAMR 0.1.18 (June 8, 2026)

- Created separate function for computing posterior summaries. This
  allows summaries to be computed with different settings from the same
  MCMC run as long as the samples are saved.

## SSLAMR 0.1.17 (May 20, 2026)

- Added two new arguments to the simulation function. 1) `vif` allows
  data to be simulated from negative binomial distributions. This
  parameter represents the ratio of the variance to the mean and must be
  greater than or equal to 1. If equal to 1, then counts are Poisson. 2)
